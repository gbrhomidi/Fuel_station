 ...
 * ═══════════════════════════════════════════════════════════════
 *
 * التنفيذ الفعلي لـ StationRepository باستخدام DatabaseHelper الحالي.
 * يعمل كـ Adapter/Wraper حول SQLiteOpenHelper الحالي بدون تعديله.
 *
 * الهدف: استبدال NanoHTTPD REST API بـ Kotlin Coroutines + Repository Pattern
 */
class StationRepositoryImpl(
    ...
    private inline fun <T> safeDbCall(block: () -> T): Result<T> = try {
                ...
                if (user != null) {
                    AuthResult(
                        ...
                        user = User(
                            ...
                            roleId = user.optInt("role_id").takeIf { it > 0 }
                        ),
                        token = UUID.randomUUID().toString()
                    )
                } else {
                    AuthResult(success = false, error = "بيانات خاطئة")
                }
            } catch (e: Exception) {
    ...
    override suspend fun deleteUser(userId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE users SET is_deleted = 1 WHERE id = ?", arrayOf(userId.toString()))
            true
        }
    }

    override suspend fun deleteRole(roleId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("DELETE FROM roles WHERE id = ?", arrayOf(roleId.toString()))
            true
        }
    }
    ...
    // PARTIES (CUSTOMERS)
    // ================================================================
    override suspend fun getCustomers(): Result<List<Party>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun getCustomerById(id: Int): Result<Party?> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            db.rawQuery("SELECT * FROM parties WHERE id = ? AND (is_deleted = 0 OR is_deleted IS NULL)",
    ...
    override suspend fun addCustomer(party: Party): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
    ...
    override suspend fun updateCustomer(party: Party): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
                put("name", party.name)
    ...
    override suspend fun deleteCustomer(partyId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE parties SET is_deleted = 1 WHERE id = ?", arrayOf(partyId.toString()))
            true
        }
    }
    ...
    override suspend fun getFuelTypes(): Result<List<FuelType>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun addProduct(product: Product): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
    ...
    override suspend fun updateProduct(product: Product): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
                put("name", product.name)
    ...
    override suspend fun deleteProduct(productId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE products SET is_deleted = 1 WHERE id = ?", arrayOf(productId.toString()))
            true
        }
    }
    ...
    // TANKS & PUMPS
    // ================================================================
    override suspend fun getTanks(): Result<List<Tank>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun updateTankQuantity(tankId: Int, quantity: Double): Result<Boolean> =
        withContext(Dispatchers.IO) {
            safeDbCall {
                val db = dbHelper.writableDatabase
                val cv = android.content.ContentValues().apply {
    ...
    // SALES
    // ================================================================
    override suspend fun getSales(): Result<List<Sale>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            val list = mutableListOf<Sale>()
            db.rawQuery("SELECT * FROM sales_transactions WHERE is_deleted = 0 OR is_deleted IS NULL ORDER BY created_at DESC", null).use { cursor ->
                while (cursor.moveToNext()) {
    ...
    override suspend fun executeSale(sale: Sale): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
                put("uuid", sale.uuid)
    ...
    override suspend fun deleteSale(saleId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE sales_transactions SET is_deleted = 1 WHERE id = ?", arrayOf(saleId.toString()))
            true
        }
    }

    override suspend fun getFuelSales(): Result<List<FuelSale>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun getDeliveries(): Result<List<Delivery>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    // FINANCE
    // ================================================================
    override suspend fun getPayments(): Result<List<Payment>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            val list = mutableListOf<Payment>()
            db.rawQuery("SELECT * FROM payments WHERE is_deleted = 0 OR is_deleted IS NULL ORDER BY created_at DESC", null).use { cursor ->
                while (cursor.moveToNext()) {
    ...
    override suspend fun makePayment(payment: Payment): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
    ...
    override suspend fun addDeposit(deposit: Deposit): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
    ...
    override suspend fun deletePayment(paymentId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE payments SET is_deleted = 1 WHERE id = ?", arrayOf(paymentId.toString()))
            true
        }
    }
    ...
    override suspend fun addEmployee(employee: Employee): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
    ...
    override suspend fun updateEmployee(employee: Employee): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
                put("full_name", employee.fullName)
    ...
    override suspend fun deleteEmployee(employeeId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE employees SET is_deleted = 1 WHERE id = ?", arrayOf(employeeId.toString()))
            true
        }
    }

    override suspend fun getShifts(): Result<List<Shift>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            val list = mutableListOf<Shift>()
            db.rawQuery("SELECT * FROM shifts WHERE is_deleted = 0 OR is_deleted IS NULL ORDER BY created_at DESC", null).use { cursor ->
                while (cursor.moveToNext()) {
    ...
    override suspend fun getCashiers(): Result<List<Employee>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            val list = mutableListOf<Employee>()
            db.rawQuery(
                "SELECT * FROM employees WHERE (is_deleted = 0 OR is_deleted IS NULL) AND position LIKE ?",
                arrayOf("%كاشير%", "%cashier%")
            ).use { cursor ->
                while (cursor.moveToNext()) {
                    list.add(cursor.toEmployee())
                }
    ...
    override suspend fun addShift(shift: Shift): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
                put("uuid", shift.uuid)
    ...
    override suspend fun deleteShift(shiftId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE shifts SET is_deleted = 1 WHERE id = ?", arrayOf(shiftId.toString()))
            true
        }
    }
    ...
    // MAINTENANCE
    // ================================================================
    override suspend fun getMaintenanceRequests(): Result<List<MaintenanceRequest>> =
        withContext(Dispatchers.IO) {
            safeDbCall {
                val db = dbHelper.readableDatabase
                val list = mutableListOf<MaintenanceRequest>()
                db.rawQuery("SELECT * FROM maintenance_requests WHERE is_deleted = 0 OR is_deleted IS NULL ORDER BY created_at DESC", null).use { cursor ->
                    while (cursor.moveToNext()) {
    ...
    override suspend fun addMaintenanceRequest(request: MaintenanceRequest): Result<Int> =
        withContext(Dispatchers.IO) {
            safeDbCall {
                val db = dbHelper.writableDatabase
                val cv = android.content.ContentValues().apply {
    ...
    override suspend fun updateMaintenanceStatus(requestId: Int, status: String): Result<Boolean> =
        withContext(Dispatchers.IO) {
            safeDbCall {
                val db = dbHelper.writableDatabase
                val cv = android.content.ContentValues().apply {
    ...
    override suspend fun deleteMaintenance(requestId: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.execSQL("UPDATE maintenance_requests SET is_deleted = 1 WHERE id = ?", arrayOf(requestId.toString()))
            true
        }
    }
    ...
    // SMS & NOTIFICATIONS
    // ================================================================
    override suspend fun getSmsLogs(): Result<List<SmsMessage>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun getWhitelist(): Result<List<SmsWhitelistEntry>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun addWhitelist(entry: SmsWhitelistEntry): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
    ...
    override suspend fun removeWhitelist(phone: String): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            db.delete("sms_whitelist", "phone = ?", arrayOf(phone))
            true
        }
    }
    ...
    // SETTINGS
    // ================================================================
    override suspend fun getSetting(key: String): Result<String?> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun setSetting(key: String, value: String): Result<Boolean> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
                put("key", key)
    ...
    override suspend fun getAllSettings(): Result<Map<String, String>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    // REPORTS & DASHBOARD
    // ================================================================
    override suspend fun getDashboard(): Result<DashboardData> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            ...
            val weekStr = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(weekAgo.time)
            val monthStr = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(monthAgo.time)

            DashboardData(
                totalSales = db.rawQuery("SELECT COALESCE(SUM(total_amount), 0) as total FROM sales_transactions WHERE is_deleted = 0 OR is_deleted IS NULL", null)
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                totalTransactions = db.rawQuery("SELECT COUNT(*) FROM sales_transactions WHERE is_deleted = 0 OR is_deleted IS NULL", null)
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 },
                totalCustomers = db.rawQuery("SELECT COUNT(*) FROM parties WHERE is_deleted = 0 OR is_deleted IS NULL", null)
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 },
                totalFuelDispensed = db.rawQuery("SELECT COALESCE(SUM(quantity), 0) FROM fuel_sales WHERE is_deleted = 0 OR is_deleted IS NULL", null)
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                activeShifts = db.rawQuery("SELECT COUNT(*) FROM shifts WHERE status = 'open' AND (is_deleted = 0 OR is_deleted IS NULL)", null)
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 },
                lowStockItems = db.rawQuery("SELECT COUNT(*) FROM products WHERE quantity <= min_stock AND (is_deleted = 0 OR is_deleted IS NULL)", null)
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 },
                pendingMaintenance = db.rawQuery("SELECT COUNT(*) FROM maintenance_requests WHERE status = 'pending' AND (is_deleted = 0 OR is_deleted IS NULL)", null)
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 },
                todayRevenue = db.rawQuery("SELECT COALESCE(SUM(total_amount), 0) FROM sales_transactions WHERE DATE(created_at) = ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(today))
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                weeklyRevenue = db.rawQuery("SELECT COALESCE(SUM(total_amount), 0) FROM sales_transactions WHERE DATE(created_at) >= ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(weekStr))
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                monthlyRevenue = db.rawQuery("SELECT COALESCE(SUM(total_amount), 0) FROM sales_transactions WHERE DATE(created_at) >= ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(monthStr))
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 }
            )
        }
    }

    override suspend fun getDailySales(): Result<List<DailySales>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun getAiInsight(): Result<AiInsight> = withContext(Dispatchers.IO) {
        safeDbCall {
            // Placeholder: يمكن ربطه بـ GeminiAIHelper لاحقاً
            AiInsight(
                insight = "التحليل متاح بعد ربط خدمة AI",
                recommendations = listOf("تحقق من اتصال الإنترنت", "تأكد من إعداد مفتاح Gemini API")
            )
        }
    }

    override suspend fun getEodReport(): Result<EodReport> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            val today = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date())
            EodReport(
                date = today,
                totalSales = db.rawQuery("SELECT COALESCE(SUM(total_amount), 0) FROM sales_transactions WHERE DATE(created_at) = ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(today))
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                totalPayments = db.rawQuery("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE DATE(created_at) = ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(today))
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                totalDeposits = db.rawQuery("SELECT COALESCE(SUM(amount), 0) FROM deposits WHERE DATE(created_at) = ?", arrayOf(today))
                    .use { if (it.moveToFirst()) it.getDouble(0) else 0.0 },
                cashInHand = 0.0, // يحتاج حساب دقيق
                shiftCount = db.rawQuery("SELECT COUNT(*) FROM shifts WHERE DATE(created_at) = ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(today))
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 },
                transactionCount = db.rawQuery("SELECT COUNT(*) FROM sales_transactions WHERE DATE(created_at) = ? AND (is_deleted = 0 OR is_deleted IS NULL)", arrayOf(today))
                    .use { if (it.moveToFirst()) it.getInt(0) else 0 }
            )
        }
    }

    override suspend fun getMonthlySales(): Result<List<MonthlySales>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
    ...
    override suspend fun getProfitReport(fromDate: String, toDate: String): Result<ProfitReport> =
        withContext(Dispatchers.IO) {
            safeDbCall {
                val db = dbHelper.readableDatabase
                ...
                val cost = db.rawQuery(
                    """SELECT COALESCE(SUM(st.quantity * p.cost_price), 0) 
                       ...
                       WHERE DATE(st.created_at) BETWEEN ? AND ? AND (st.is_deleted = 0 OR st.is_deleted IS NULL)""",
                    arrayOf(fromDate, toDate)
                ).use { if (it.moveToFirst()) it.getDouble(0) else 0.0 }
    ...
    override suspend fun exportData(): Result<ExportData> = withContext(Dispatchers.IO) {
        safeDbCall {
            ExportData(
                parties = getCustomers().getOrDefault(emptyList()),
                products = getProducts().getOrDefault(emptyList()),
                sales = getSales().getOrDefault(emptyList()),
                payments = getPayments().getOrDefault(emptyList()),
                employees = getEmployees().getOrDefault(emptyList()),
    ...
    // AI
    // ================================================================
    override suspend fun aiChat(message: String, sessionId: String): Result<AiChatResponse> =
        withContext(Dispatchers.IO) {
            safeDbCall {
                // Placeholder: سيتم ربطه بـ GeminiAIHelper في المرحلة التالية
                AiChatResponse(
                    response = "الرد الذكي قيد التطوير. الرسالة المستلمة: $message",
                    sessionId = sessionId
                )
            }
        }
    ...
    // STOCK MOVEMENTS
    // ================================================================
    override suspend fun getStockMovements(): Result<List<StockMovement>> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.readableDatabase
            val list = mutableListOf<StockMovement>()
            db.rawQuery("SELECT * FROM stock_movements WHERE is_deleted = 0 OR is_deleted IS NULL ORDER BY created_at DESC", null).use { cursor ->
                while (cursor.moveToNext()) {
    ...
    override suspend fun addStockMovement(movement: StockMovement): Result<Int> = withContext(Dispatchers.IO) {
        safeDbCall {
            val db = dbHelper.writableDatabase
            val cv = android.content.ContentValues().apply {
    ...
    // ================================================================
    // Cursor Extension Functions (Mapping)
    // ================================================================
    ...
    private fun Cursor.toPump() = Pump(
        id = getInt("id"),
        uuid = getString("uuid"),
        name = getString("name"),
        tankId = getIntOrNull("tank_id"),
        fuelTypeId = getIntOrNull("fuel_type_id"),
        status = getStringOrNull("status") ?: "active",
        totalDispensed = getDoubleOrNull("total_dispensed") ?: 0.0,
        lastMaintenance = getStringOrNull("last_maintenance")
    )
